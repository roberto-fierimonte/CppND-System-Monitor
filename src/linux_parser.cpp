...
vector<int> LinuxParser::Pids() {
  ...
  for (std::filesystem::directory_entry const& dir_entry : std::filesystem::directory_iterator(kProcDirectory)) {
    if ((dir_entry.is_directory()) && (std::all_of(dir_entry.path().filename().string().begin(), dir_entry.path().filename().string().end(), isdigit))) {
...
long LinuxParser::UpTime() { 
  ...
  return (long) stof(uptime1) + stof(uptime2);
}
